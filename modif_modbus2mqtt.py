...
parser.add_argument('--fname', default='modbus.log', help='Name of the log file')
...
class Register:
    ...
    def poll(self):
        try:
            ...
            if self.format[1]:
                r=self.format[1] % r
	            #logging.info('Time-lasttime')
	            #logging.info(time.time() - self.last)
            if r!=self.lastval or (args.force and (time.time() - self.last) > int(args.force)):
                self.lastval=r
                fulltopic=topic+self.topic
                logger.info("Publishing " + fulltopic)
                mqc.publish(fulltopic,self.lastval,qos=0,retain=True)
                self.last = time.time()
        except modbus_tk.modbus.ModbusError as exc:
            logger.error("Error reading "+self.topic+": Slave returned %s - %s", exc, exc.get_exception_code())
        except Exception as exc:
            logger.error("Error reading "+self.topic+": %s", exc)
...
logger.info('Read %u valid register definitions from \"%s\"' %(len(registers), args.registers))
...
def messagehandler(mqc,userdata,msg):
    ...
    try:
        ...
        if int(slaveid) not in range(0,255):
            logger.warning("on message - invalid slaveid " + msg.topic)
            return

        if not (int(register) >= 0 and int(register) < sys.maxint):
            logger.warning("on message - invalid register " + msg.topic)
            return

        if functioncode == str(cst.WRITE_SINGLE_COIL):
            logger.info("Writing single coil " + register)
        elif functioncode == str(cst.WRITE_SINGLE_REGISTER):
            logger.info("Writing single register " + register)
        else:
            logger.error("Error attempting to write - invalid function code " + msg.topic)
            return
...
def disconnecthandler(mqc,userdata,rc):
    logger.warning("Disconnected from MQTT broker with rc=%d" % (rc))

tryToConnect = False
while tryToConnect == False: #loop until it's connected
    try:
        ...
        if args.rtu:
            ...
            logger.error("You must specify a modbus access method, either --rtu or --tcp")
            sys.exit(1)
